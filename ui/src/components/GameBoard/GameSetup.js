...
const GameSetup = ({ gameSetupProps }) => {
    const { stompClientRef, shipInfo, selectedShip, shipDirection, placedShipCount,
    ...
    const [localShipInfo, setLocalShipInfo] = useState(shipInfo);

    useEffect(() => {
        setLocalShipInfo(shipInfo);
    }, [shipInfo]);
